...
class _InventoryDashboardState extends State<InventoryDashboard> {
  ...
  final List<String> _categoriesSw = [
    "Zote",
    "Vinywaji",
    "Chakula na Nafaka",
    "Matunda na Mboga",
  ...
  Widget build(BuildContext context) {
    ...
    final categories = appState.locale.languageCode == 'sw'
        ? _categoriesSw
        : _categoriesEn;
    ...
    return Scaffold(
      ...
      body: isFirebaseInitialized
          ? _buildFirebaseContent(appState, categories)
          : _buildDemoContent(appState, categories),
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: Text(
          appState.translate('add_product'),
  ...
  Future<void> _handleBulkUpload() async {
    ...
    if (products != null && products.isNotEmpty) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("No products found. Use format: Name, Category, Qty"),
    ...
    if (scannedCode != null && scannedCode.isNotEmpty) {
      if (isFirebaseInitialized && FirebaseAuth.instance.currentUser != null) {
        ...
        if (query.exists) {
          ...
          if (mounted) {
            _showItemForm(context, appState, initialBarcode: scannedCode);
          }
        }
      } else {
        // Demo Mode Search
  ...
  Widget _buildFirebaseContent(
    ...
    return StreamBuilder<DataSnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final docs =
            (snapshot.data?.value as Map<dynamic, dynamic>?)?.entries ?? [];
        final allData = docs
            .map(
              (d) => {
                ...Map<String, dynamic>.from(d.value as Map),
                'id': d.key,
              },
            )
            .toList();
  ...
  Widget _buildMainLayout(
    ...
    int low = items
        .where((i) => (i['quantity'] ?? 0) < 5 && (i['quantity'] ?? 0) > 0)
        .length;
  ...
  Widget _buildStatsSummary(
    ...
    return Container(
      ...
      child: ListView(
        ...
        children: [
          _buildMiniStat(
            appState.translate('total_items'),
            total.toString(),
            Icons.inventory_2_rounded,
            Colors.blue,
          ),
          _buildMiniStat(
            appState.translate('low_stock'),
            low.toString(),
            Icons.trending_down_rounded,
            Colors.orange,
          ),
          _buildMiniStat(
            appState.translate('out_stock'),
            out.toString(),
            Icons.error_rounded,
            Colors.red,
          ),
          ...
          Expanded(
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                filled: true,
                fillColor: appState.themeMode == ThemeMode.dark
  ...
  Widget _buildItemCard(
    ...
    Color statusColor = qty == 0
        ? Colors.red
        : (qty < 5 ? Colors.orange : Colors.green);
    final isDark = appState.themeMode == ThemeMode.dark;
    ...
    return Container(
      ...
      child: ListTile(
        onTap: () =>
            _showItemForm(context, appState, itemData: data, itemId: id),
  ...
  void _showItemForm(
    ...
    final barcodeController = TextEditingController(
      text: itemData?['barcode'] ?? initialBarcode ?? "",
    );
    final categories = appState.locale.languageCode == 'sw'
        ? _categoriesSw
        : _categoriesEn;
    String selectedCat =
        itemData?['category'] ?? categories[categories.length - 1];
  ...
  Future<void> _saveItemSilently({
    ...
    if (isFirebaseInitialized && FirebaseAuth.instance.currentUser != null) {
      await FirebaseDatabase.instance.ref('products').push().set(data);
    } else {
